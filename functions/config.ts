...
/** Read all config from D1 cache. Returns null if cache is empty or all entries are stale. */
...
/** Write all config rows to D1 cache (upsert). */
async function writeToD1(db: D1Database, config: Record<string, string>): Promise<void> {
  ...
  const stmts = Object.entries(config).map(([key, value]) =>
    db
      .prepare(
        'INSERT INTO config_cache (key, value, cached_at) VALUES (?, ?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value, cached_at = excluded.cached_at',
      )
      .bind(key, value, now),
  );
  // Batch all upserts in a single D1 transaction
...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  ...
  // 3. Write back to D1 (non-blocking — don't await, don't fail the response)
  if (env.CONFIG_CACHE) {
